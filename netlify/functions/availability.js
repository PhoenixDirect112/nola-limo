...
//
//  Security:
//  • All credentials are env vars, never in source
//  • Rate limiting via in-memory store (replace with Redis in prod)
//  • CORS restricted to same origin
//  • Response does not expose calendar event titles/details
// ─────────────────────────────────────────────────────────────────
...
// ── Get Microsoft OAuth2 access token (Client Credentials) ──────
...
async function getCalendarEvents(token, startISO, endISO) {
  ...
  const params = new URLSearchParams({
    ...
    endDateTime:   endISO,
    '$select':     'start,end',    // Only fetch start/end — no titles/details
    '$top':        '100',
  });
...
// ── Convert events to booked time slots ─────────────────────────
function eventsToBookedSlots(events) {
  ...
  for (const event of events) {
    const start = new Date(event.start.dateTime + (event.start.dateTime.endsWith('Z') ? '' : '-06:00'));
    const end   = new Date(event.end.dateTime   + (event.end.dateTime.endsWith('Z')   ? '' : '-06:00'));

    // Mark all hourly slots that overlap this event as booked
    for (const time of ALL_TIMES) {
      ...
      // Overlap check: slot overlaps event if slot.start < event.end && slot.end > event.start
      if (slotStart < end && slotEnd > start) {
        const dateKey = `${start.getFullYear()}-${String(start.getMonth()+1).padStart(2,'0')}-${String(slotStart.getDate()).padStart(2,'0')}`;
...
exports.handler = async function(event, context) {
  ...
  try {
    ...
    return {
      statusCode: 200,   // Fail open: return empty slots so booking still works
      headers,
      body: JSON.stringify({ bookedSlots: {}, _notice: 'Calendar temporarily unavailable' }),
    };
  }
};
